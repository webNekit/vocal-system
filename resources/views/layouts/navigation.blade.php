                ...
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        ...
                        @if(Auth::user()?->role === 'admin')
                            ...
                            <x-nav-link :href="route('admin::assignment.index')" :active="request()->routeIs('admin::assignment.index')">
                                {{ __('Наставничество') }}
                            </x-nav-link>
                        @endif
                        @if(Auth::user()->role === 'user')
                            <x-nav-link :href="route('user::talks.index')" :active="request()->routeIs('user::talks.index')">
                                {{ __('Обсуждения') }}
                            </x-nav-link>
                        @endif
                        @if(Auth::user()->role === 'mentor')
                            <x-nav-link :href="route('mentor::talk.index')" :active="request()->routeIs('mentor::talk.index')">
                                {{ __('Обсуждения') }}
                            </x-nav-link>
                        @endif
                    </div>
